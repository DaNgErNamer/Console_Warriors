...
internal class Actions
{
    ...
    //}
    #endregion
    static float Calculate_DamageThroughtArmor(Units defender, float Damage, string attack_type) // —тарый скрипт рассчета урона через броню
    {
    ...
    static float Calculate_ArmorDestruction(Units defender, float Damage, string attack_type) // —тарый скрипт рассчета урона по броне
    {
        float Armor = 0;
    ...
    static float Calcultate_ShieldDamage(float damage, Units defender) // –ассчет урона по щиту
    {
        int damage_int = Convert.ToInt32(damage);
        int damage_throgh_shield = 0;
        if (defender.shield != 0) //≈сли щита вовсе нет
        {
