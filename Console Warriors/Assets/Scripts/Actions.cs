...
internal class Actions
{
    ...
    internal class LightAttack : BaseAction
    {
        ...
        public override bool DoAction(Actor attacker, Actor defender)
        {
            ...
            if (!IsTargetEvaded(defender))
            {
                ...
                /// ѕланируемое отключение системы разрущени€ брони
    ...
    internal class TryToEvade : BaseAction
	{
        ...
        public override bool DoAction(Actor actor)
        {
            if (!CheckEnergy(actor, cost)) return false;
            actor.unit.energy -= cost;
            actor.AddEffect(new Effects.EvasionBoost(actor,40,4,actor.effects_prefs.Evasion_pref));
    ...
    static float Calculate_DamageThroughtArmor(Actor defender, float Damage, string attack_type) // —тарый скрипт рассчета урона через броню
    {
        ...
        /// —тарые формулы рассчета урона через броню, устарели
        ...
        else if (attack_type == "Heavy") // —ледует потом изменить
        {
            pierce_Damage = (Damage) * (1 - ((float)defender.unit.armor / 100));
        }
    ...
    /// <summary>
    /// ќтключенный метод расчета разрушени€ брони, планируетс€ убрать механику
    /// </summary>
